...
export default function Progressbar({ className, value, setValue, func }) {
    ...
    useEffect(() => {
        const handleMouseUp = (event) => {
            ...
            setScrub(null);
            if (
                !event.target.classList.contains("progress_bar") &&
                !event.target.classList.contains("progress_bar_progress") &&
                !event.target.classList.contains("progress_bar_slider")
            ) {
                setEngaged(false);
            }
        };
        ...
        >
            ...
            <button
                className={`progress_bar_slider ${
                    engaged && "progress_bar_slider_engaged"
                } ${className}`}
                style={{
                    left:
                        className === "progress_volume"
                            ? `${((scrub || value) * 100 + 1).toFixed(2)}%`
                            : `${((scrub || value) * 100).toFixed(2)}%`,
                }}
            />
        </Wrapper>
    );
